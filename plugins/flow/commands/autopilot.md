...
`/flow:pause land`'s verification decision (its Step 4) never prompts: with `stop_check` unset or `ask`, a `land` runs verification automatically (or reuses a green run from this session when nothing changed since). `always`/`never` in config are honoured as configured, also without prompting.
